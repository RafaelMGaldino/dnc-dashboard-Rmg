    ...
    <>
      ...
      <Container maxWidth="lg">
        <Grid container sx={{ width: '100%', gap: 4, alignItems: 'stretch' }}>
          ...
          <Grid component="div"sx={{whiteSpaceTrimidth: { xs: '100%', md: '65%' }, display: 'flex', alignItems: 'center',  justifyContent: 'center',}}>
            {
                !salesYearError &&(
                 <CardComponent className={salesYearLoading ? 'skeleton-loading skeleton-loading-mh-2' : ''}>
                  
                  
                 {!salesStarLoading && salesYearData && (

                  <>
                    <StyledH3 className="mb-1">Valor de Vendas No Mês</StyledH3>
                    <CustomChart
                    labels={salesYearData.labels.map((label) => label)}
                    data={salesYearData.data.map((value) => value)}
